...
func init() {
	...
	if config.ExistConfigFile() && config.GetValueBoolDefault("redis.enable", false) {
		err := config.GetValueObject("redis", &config.RedisCfg)
		if err != nil {
